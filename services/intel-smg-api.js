...
async function getAccessToken() {
  ...
  if (refreshToken) {
    ...
    throw new Error(`SMG refresh token failed: ${res.status} ${res.body.slice(0,300)} — re-seed SMG_REFRESH_TOKEN in Render env vars`);
  }

  throw new Error('SMG_REFRESH_TOKEN not configured — add to Render env vars');
}
